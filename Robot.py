...
class Robot:
    def __init__(
        ...
        self.has_left_initial_waypoint = (
            False  # True once it moves from/rotates at the first WP
        )
        self.patrol_cycle_completed = (
            False  # True once it returns to the first WP after starting
        )
        ...
        if self.waypoints.size > 0:
            ...
            if np.linalg.norm(self.position - first_target_wp) < EPSILON:
                if len(self.waypoints) > 1:  # If at WP0 and there's a WP1
                    ...
                    if (
                        np.linalg.norm(direction_vector_to_next) > EPSILON
                    ):  # Check if next WP is different
        ...
        self.departure_orientation = self.normalize_angle(
            self.orientation
        )  # Initial departure is current orientation
    ...
    def move(self):
        ...
        if self.patrol_cycle_completed or not self.waypoints.size:
            ...
            if (
                ...
                or self.path_history[-1][1] != self.sensed_signal_strength
            ):
        ...
        if self.is_rotating:
            ...
            if abs(angle_difference) < ORIENTATION_ALIGNMENT_THRESHOLD_RAD:
                self.orientation = self.normalize_angle(
                    self.departure_orientation
                )  # Snap to final orientation
                self.is_rotating = False
            ...
            if distance_to_waypoint < WAYPOINT_REACH_THRESHOLD:  # Reached a waypoint
                ...
                if (
                    ...
                    if current_wp_is_initial and not self.has_left_initial_waypoint:
                        self.has_left_initial_waypoint = (
                            True  # Considered "left" if moving on from initial WP
                        )
                    ...
                    current_orientation_normalized = self.normalize_angle(
                        self.orientation
                    )
            ...
            ):  # Moving towards the current target_waypoint
    ...
    def _append_to_path_history(self):
        ...
        if (
            ...
            self.path_history.append(
                (self.position.copy(), self.sensed_signal_strength)
            )
            if (
    ...
    def sense_signal(self, signal_source_pos, emitter_params):
        ...
        def angle_to_point(agent_pos, agent_theta, target_pos):
            ...
            return angle_diff  # in radians, [-π, π]
        ...
        pr = (
            ...
            + convert_to_dB(wavelength / (4 * np.pi * distance + EPSILON))
            + noise
        )
    ...
    def update_patrol(self, new_waypoints):
            ...
            self.is_rotating = False  # Ensure it's not stuck in a rotating state

            # Define the start/end point for the new patrol cycle.
            if self.waypoints.size > 0:
                self.initial_patrol_waypoint_pos = self.waypoints[0].copy()
                # Reset completion logic for the new patrol.
                self.has_left_initial_waypoint = False
            else:
                # An empty patrol is considered complete.
