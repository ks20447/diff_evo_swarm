...
def voronoi_cells(map_polygon):
    ...
    x_coords = np.linspace(minx, maxx, 3)
    y_coords = np.linspace(miny, maxy, 3)
...
def run_simulation(cell_id, plots):
    robots = initialize_robots(robot_waypoints)
    simulation_data = []
    source_info = {"pos": SIGNAL_SOURCE_POS, "strength": SIGNAL_POWER}
...
def plot_orthogonal_line(ax, max_point, edge_start, edge_end, length=0.5, lw=2, color='cyan'):
    x0, y0 = max_point
    x1, y1 = edge_start
    x2, y2 = edge_end
...
if __name__ == "__main__":
    ...
    for i, cell in enumerate(cells):
        ...
        for patrol in patrols:
            print(list(patrol.patrol.exterior.coords))
        ...
        robot_data = run_simulation(i, plots)
