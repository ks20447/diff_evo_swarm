...
class Simulator:
    ...
    def initialize_plot(self, cell): # Assuming 'cell' is a shapely geometry or similar
        ...
        if hasattr(cell, 'exterior'): # Check if cell has an exterior (like a polygon)
            self.ax.plot(cell.exterior.xy[0], cell.exterior.xy[1], color="r", label="Cell Boundary")
    ...
    def initialise(self):
        ...
        for i in range(self.num_robots):
            ...
            if i < len(self.waypoints):
                 ...
                 waypoints_for_robot = np.array([]) # Default to empty if not enough waypoints defined

            initial_pos = waypoints_for_robot[0] if len(waypoints_for_robot) > 0 else None
    ...
    def add_robots_to_plot(self):
        ...
        colors = ["r", "g", "b", "c", "m", "y", "orange", "purple", "brown", "pink"] # Expanded colors
        ...
        for i in range(self.num_robots):
            ...
            if i < len(self.waypoints) and self.waypoints[i] is not None and self.waypoints[i].size > 0:
        ...
        all_points_for_limits = list(self.map_vertices.reshape(-1, 2)) if self.map_vertices.size > 0 else []
        ...
        if all_points_for_limits:
            ...
            if all_points_np.ndim == 2 and all_points_np.shape[0] > 0:
                ...
                x_padding = max(1.0, (x_max - x_min) * 0.1) # Add 10% padding or at least 1 unit
        ...
        for i, robot in enumerate(self.robots):
            ...
            if len(robot.path_history) >= 2:
                ...
                for j in range(1, len(robot.path_history)):
                    pos_prev, _ = robot.path_history[j - 1]
                    pos_curr, sig_curr_raw = robot.path_history[j] # sig_curr_raw can be scalar or array
                ...
                robot_trail_collections[i].set_array(np.array(segment_signal_values, dtype=float))
            else:
            ...
            state_str = (
                "Done" if robot.patrol_cycle_completed else ("Rot" if robot.is_rotating else "Mov")
            )
            
            current_signal_raw = robot.sensed_signal_strength
            signal_display_val = 0.0 # Default display value
            if isinstance(current_signal_raw, np.ndarray) and current_signal_raw.size > 0:
                signal_display_val = current_signal_raw[1] # Display the first element
            elif isinstance(current_signal_raw, (float, int, np.number)): # Check for scalar numeric types
    ...
    def run(self, cell, robot_waypoints):
        ...
        cell_polygon = cell.get("cell_polygon", None) # This should be a geometry object for initialize_plot

        self.initialise() # Initialise robots with their waypoints

        if self.plotting:
            if cell_polygon is None:
                print("Warning: cell_polygon not provided, skipping cell boundary plot.")
        ...
        if self.step_count == self.sim_steps and not self.all_robots_finished_patrol:
            print(f"Simulation ended after {self.sim_steps} steps; not all robots finished.")
            ...
            self.sim_data.append(
                {
                    ...
                    # Convert path_history to list of tuples for JSON serializability if needed later
                    # Robot path_history stores (np.array_pos, signal_value)
                    # For signal value that might be np.array, ensure it's converted if needed for storage
                    "path_history": [(pos.tolist(), sig.tolist() if isinstance(sig, np.ndarray) else sig) 
                                     for pos, sig in robot.path_history],
                }
            )
